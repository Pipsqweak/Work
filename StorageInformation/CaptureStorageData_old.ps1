                ...
                $_.Volumes | ForEach-Object {
                    $_.Datastores | ForEach-Object {
                        $_.VirtualMachines | Select-Object `
                            ...
                            @{N="Host";E={ $_.VMHost.Name -replace ".powereng.com","" }},
                            @{N='GPUProfile';E={$p = $_.ExtensionData.Config.Hardware.Device | Where-Object { $_.Backing.vgpu } | Select-Object -ExpandProperty DeviceInfo | Select-Object -ExpandProperty Summary; if($null -ne $p) { $p = $p -replace "NVIDIA", "" -replace "grid","" -replace "vgpu",""; $p = $p.Trim(@(' ','_')) }; $p }}
                    }
                }
            }
        } `
...
$requiredFiles = @(
    ...
    "StorageInfoClasses\StorageInfoClasses_old.cs",  # C# source code to define classes used in this script
