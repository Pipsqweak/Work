...
#
# Copy the following lines into the calling script prior to sourcing this file.
...
if(@([System.AppDomain]::CurrentDomain.GetAssemblies() | Where-Object { $_.FullName -match "MySQL.Data" }).Length -gt 0)
{
    try { $found = ($null -ne [MySQLDBConnection]) } catch { $found = $false }
    if(-not $found)
    {
        # Define the database connection class
        Class MySQLDBConnection : System.IDisposable
        {
            static [Int32] $maxRetries = 5
            # The underlying database connection object used by the other methods of the class

            [MySql.Data.MySqlClient.MySqlConnection] hidden $connection
            ...
            )
            {
                ...
                {
                    $this.connection = [MySql.Data.MySqlClient.MySqlConnection]::new($connectionString)
            ...
            [Boolean] ExecuteLoad([MySql.Data.MySqlClient.MySqlCommand] $cmd, [System.Data.DataTable] $tmpDT)
            {
                #
                #    I discovered when multiple scripts were trying to make the same query, the following exception would occur:
                #
                #       Exception calling "Load" with "1" argument(s): "Transaction (Process ID 234) was deadlocked on lock resources with
