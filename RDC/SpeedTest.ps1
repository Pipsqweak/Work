...
function SummarizeTestResults
{
    ...
    if($readTimeMS.Sum -gt 0)
    {
        ...
        if($d.AverageMS -ne 0)
        {
            $d.Variance = ($d.MSStdDev / $d.AverageMS) * 100.0
        }
    }
...
function TestDatacenter
{
    ...
    if($alive)
    {
        ...
        if($alive -and ($null -ne $tr))
        {
            ...
            $testResults = "" | Select-Object Office, DCName, Description, TestHost, DateTime, ServerName, ShareName, Latency, FileCopy, FileTestSummary
            $testResults.FileTestSummary = "" | Select-Object Small, Medium, Large

            $testResults.Office = $office
            $testResults.DCName = $dcName
            $testResults.Description = $description
            $testResults.TestHost = $env:COMPUTERNAME
            ...
            else
            {
                ...
                Write-Host ("`r`nSmall file read time average: {0:N2}, standard deviation: {1:N2}, Variance: {2:N2}%" -f @($testResults.FileTestSummary.Small.AverageMS, $testResults.FileTestSummary.Small.MSStdDev, $testResults.FileTestSummary.Small.Variance))
                $testResults.FileTestSummary.Medium = SummarizeTestResults -fileResults $testResults.FileCopy -MediumFiles
                Write-Host ("Medium file read time average: {0:N2}, standard deviation: {1:N2}, Variance: {2:N2}%" -f @($testResults.FileTestSummary.Medium.AverageMS, $testResults.FileTestSummary.Medium.MSStdDev, $testResults.FileTestSummary.Medium.Variance))
                $testResults.FileTestSummary.Large = SummarizeTestResults -fileResults $testResults.FileCopy -LargeFiles
                Write-Host ("Large file read time average: {0:N2}, standard deviation: {1:N2}, Variance: {2:N2}%`r`n" -f @($testResults.FileTestSummary.Large.AverageMS, $testResults.FileTestSummary.Large.MSStdDev, $testResults.FileTestSummary.Large.Variance))
            }
        }
        else
        {
...
"@ -f @($officeName)
    ...
    $allTestResults  | ForEach-Object { $outputResults.Add($_) }

    if(-not [String]::IsNullOrEmpty($preSDWANResultsFileName) -and [System.IO.File]::Exists($preSDWANResultsFileName))
    {
    ...
    try
    {
        $recipients = @("Carlos.Gonzales@powereng.com", "ken.briney@powereng.com", "nate.mcghehey@powereng.com")
        ...
        if([System.IO.Directory]::Exists($off2RDCMetricsConfig.ResultsRepository))
        {
            ...
            if ($Riverbed)
            {
                ...
                while($a -lt $off2RDCMetricsConfig.EDCs.Length)
                {
                    ...
                    {
                        $tr = TestDatacenter -office $hostOffice.Name -dcName $off2RDCMetricsConfig.EDCs[$a].Name -description $Description -serverName $off2RDCMetricsConfig.EDCs[$a].ServerName -shareName $off2RDCMetricsConfig.EDCs[$a].ShareName -TrackOverallElapsed:$TrackOverallElapsed -LimitLargeFiles:$LimitLargeFiles
                        if($null -ne $tr)
                        {
            ...
            else # NOT ($Riverbed)
            {
                ...
                if($null -ne $hostRDC)
                {
                    ...
                    {
                        $tr = TestDatacenter -office $hostOffice.Name -dcName $hostOffice.Name -description $Description -serverName $hostOffice.LocalStorage.ServerName -shareName $hostOffice.LocalStorage.ShareName -TrackOverallElapsed:$TrackOverallElapsed -LimitLargeFiles:$LimitLargeFiles
                        $allTestResults.Add($tr)
                    }
                    ...
                    if($hostRDC.Active)
                    {
                        ...
                        {
                            $tr = TestDatacenter -office $hostOffice.Name -dcName $hostRDC.Name -description $Description -serverName $hostRDC.ServerName -shareName $hostRDC.ShareName -TrackOverallElapsed:$TrackOverallElapsed -LimitLargeFiles:$LimitLargeFiles
                            $allTestResults.Add($tr)
                        } `
                        else
                        {
                    ...
                    while(((-not $TrackOverallElapsed) -or ($Script:overAllSW.Elapsed.TotalMinutes -lt $Script:MaximumRuntime)) -and ($a -lt $off2RDCMetricsConfig.RDCs.Length))
                    {
                        ...
                        if($a -ne $off2RDCMetricsConfig.RDCs.IndexOf($hostRDC) -and (@($allTestResults | Where-Object { $_.DCName -eq $off2RDCMetricsConfig.RDCs[$a].Name }).Length -eq 0))
                        {
                            ...
                            {
                                $tr = TestDatacenter -office $hostOffice.Name -dcName $off2RDCMetricsConfig.RDCs[$a].Name -description $Description -serverName $off2RDCMetricsConfig.RDCs[$a].ServerName -shareName $off2RDCMetricsConfig.RDCs[$a].ShareName -TrackOverallElapsed:$TrackOverallElapsed -LimitLargeFiles:$LimitLargeFiles
                                if($null -ne $tr)
                                {
                                    $allTestResults.Add($tr)
                                }
            ...
            if($allTestResults.Count -gt 0)
            {
                ...
                $outputResults = $allTestResults |
                    Select-Object Office, DCName, TestHost, ServerName, DateTime, Description,
                        ...
                        @{N='SmallFileMSVariance';E={$_.FileTestSummary.Small.Variance}},
                        ...
                        @{N='MediumFileMSVariance';E={$_.FileTestSummary.Medium.Variance}},
                ...
                {
                    $preSDWANResultsFileName = "{0}\{1}" -f @($off2RDCMetricsConfig.ResultsRepository, $off2RDCMetricsConfig.PreSDWANResultsFile)
                }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUsePdzIxUrqFDxA8MkNjktDmu
# OhWgggyXMIIF3zCCBMegAwIBAgITFQAAAALU9Lz04Hi9mwAAAAAAAjANBgkqhkiG
...
# MBwGCisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQu
# hIuDG2aoZa/2ZcLityV4bXFpejANBgkqhkiG9w0BAQEFAASCAQBXhmAkMZ2Npym/
# JiGbzakJaMrF+MOGZ6AOJMphRVvo4JruHCUY+/RvJDifZqphOz0S+Q3kVFVi0wnj
# NFjRoQ/DXhDMZAPPWH3JzWC9dH5RTQPvDKOZ8QEsTPyObaQQNbQgG70KFsyibsP1
# CSs437iXn0/lW9HpohCbz1/UasaIxAbkONeg3ZV2zB9KgvrQZWkqP9EyrkYux4ds
# 0V6HKWXM84H5LcJ9dnftBsQeYxpfzqqXMqoOD7N8QnJpUkyQQVYYwHHlM0yWMcLB
# zZpt1+SQ7eMiDHsK5xCvuMwltiiWqLcyruUuPmhInUfbBc0wgnD4p6MXOKo+4/f0
# 1narA+v0
