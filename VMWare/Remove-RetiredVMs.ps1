...
# Version 2.1.5.0
#
# Last Revised: Monday, December 9, 2019 4:35:15 PM
# Keywords:
#
...
Function Send-FailureMessage {
    ...
    $subject = "Notification: VM deletion script failure. [Script Time:] $([math]::Round(($endDTM-$startDTM).totalminutes,2)) minutes. | Total VM(s): $VMCount"
    # Create specific message content
    $message.Subject = $subject
    $message.Body = ($message | Out-String)
...
#[string]$To = 'InfrastructureBackOffice@powereng.com,InfrastructureEngineering@powereng.com'
[string]$To = 'ITInfrastructure@powereng.com'
$SContinue = 'SilentlyContinue'
# Get Start Time
...
#$WorkingPath = 'C:\scripts\VMware'  #Local directory.
...
# KLB - 2020/02/11
# Get end of life virtual machines.  Must be tagged with "Marked-For-Deletion"
$eolVMs = @(Get-VM -Tag "Marked-For-Deletion")
...
$vmIdx = 0
while($vmIdx -lt $eolVMs)
{
    $eolVMData = "" | Select-Object VM, FQDN, DeletionDate, Requestor, AdditionalNotificationAddresses

    $eolVMData.VM = $eolVMs[$vmIdx]
    $eolVMData.FQDN = [String]::Empty
    $eolVMData.DeletionDate = [DateTime]::new(0)
    $eolVMData.Requestor = [String]::Empty

    if($eolVMData.VM.PowerState -eq "PoweredOff")
    {
        $eolVMData.FQDN = $eolVMData.VM.Name
        # If fqdn is not a fqdn, make it one.
        if($eolVMData.FQDN -notmatch "(?=^.{4,253}$)(^((?!-)[a-zA-Z0-9-]{0,62}[a-zA-Z0-9]\.)+[a-zA-Z]{2,63}$)")
        {
            $eolVMData.FQDN += ".powereng.com"
        }
        $eolVMData.FQDN = $eolVMData.FQDN.ToLower()

        Write-Logdata -message ("Getting {0}'s custom attributes" -f @($eolVMData.VM.Name))
        $annotations = @(Get-Annotation -Entity $eolVMData.VM)
        if($annotations.Length -gt 0)
        {
            $deletionDateAnnotations = @($annotations | Where-Object { $_.Name -eq "DeletionDate" })
            if($deletionDateAnnotations.Length -gt 0)
            {
                if(-not [String]::IsNullOrEmpty($deletionDateAnnotations[0].Value))
                {
                    $deletionDate = [datetime]::new(0)

                    # NOTE: I tried to parse $deletionDateAnnotations[0].Value directly into $eolVMData.DeletionDate, and even though TryParse returned $true,
                    #    I kept ending up with: Monday, January 1, 0001 12:00:00 AM as a value, so I did the parsing indirectly.
                    if([DateTime]::TryParse($deletionDateAnnotations[0].Value, [ref]$deletionDate))
                    {
                        $eolVMData.DeletionDate = $deletionDate
                    }
                }
            }

            $requestorAnnotations = @($annotations | Where-Object { $_.Name -eq "DeleteRequestedBy" })
            if($requestorAnnotations.Length -gt 0)
            {
                $eolVMData.Requestor = $requestorAnnotations[0].Value
            }
        }
    }
    else
    {
        Send-FailureMessage -message ("{0} was not deleted because it is not powered off." -f @($eolVM.Name))
    }
    $vmIdx++
}
...
foreach ($ComputerShortname in $eolSystems) {
  ...
  If ($pwCheck)
  {
      ...
      #$timespandays = ([datetime]$DeleteDate.Value - $startDTM).Days
      Write-Logdata -message "Days until deletion $(($deleteDate - $startDTM).days)"
      #Write-Host ('Systemm: {0} Day(s) until deletion: {1}' -f $eolsystem, $timespandays)
...
"@
        ...
        <#
        #$AliasCheck =  Get-DnsServerResourceRecord -Name $Arecord -ComputerName $DNSserver -RRType 'CNAME' -ZoneName powereng.com
        #$Alias = ($AliasCheck.RecordData.HostNameAlias).TrimEnd("."," ")

        #$AliasCheck.namehost -match $($ComputerShortname).powereng.com
        #>
...
"@
      ...
      $WindowAdminContent
        ...
        $PostContent = @"
      <DIV ALIGN=LEFT>Date Submitted: <b> $($RequestDate) </b></br></p></DIV><DIV ALIGN=Left>Submitted By: <b>$Requestor </b></br></DIV>
        ...
        <UL>
            ...
            <LI><p class="detail">Update IP spreadsheet <span class="name">(Pending)</span> </p>
...
"@
  ...
  Else
  {
    Send-FailureMessage -message "$ComputerShortname was not deleted because its power state wasn't powered off."
  }
} #Foreach $eolsystem
