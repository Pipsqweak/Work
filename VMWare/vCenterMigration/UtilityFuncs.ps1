...
function GetObjectChildren($obj,$pathStr)
{
    $idx = 0
    if(($null -ne $obj) -and ($obj.Name -notmatch "^vCLS-"))
    {
        ...
        if($idx -lt 0)
        {
            if($obj.Name -ne "Datacenters")
            {
                ...
                $p = 0
                do {
                    $d = "" | Select-Object Path,Name,ID,PersistentId,PermNum,Principal,Role,Propagate
                    $d.Path = $pathStr
                    $d.Name = $obj.Name
                    $d.ID = $obj.Id
                    ...
                    if($perms.Length -gt $p)
                    {
                        $d.Principal = $perms[$p].Principal
                        $d.Role = $perms[$p].Role
                        $d.Propagate = $perms[$p].Propagate
                    }
                    $exportData.Add($d)

                    $p++
                } while($p -lt $perms.Length)
            }
    ...
    try
    {
        if(($idx -lt 0) -or ($null -eq $obj))
        {
            ...
            while($a -lt $children.Length)
            {
                ...
                GetObjectChildren $children[$a] $pathStr2
                $a++
            }
        }
    }
