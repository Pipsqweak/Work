...
function NewActionSequenceCommand
{
    ...
    Write-Host ("{0}" -f @($funcParameters["callee"]))
    ...
    if($null -ne $tParams)
    {
        ...
        if($paramNames -contains "callee")
        {
            ...
            while($a -lt $paramNames.Length)
            {
                ...
                {
                    { ($_ -eq [NetApp.Ontapi.Filer.C.NcController]) -or
                      ($_ -eq [DataONTAP.C.Types.Volume.VolumeAttributes]) -or
                      ($_ -eq [DataONTAP.C.Types.Cifs.CifsServerConfig]) -or
                      ($_ -eq [DataONTAP.C.Types.Snapmirror.SnapmirrorInfo])
                    }                                                             { $tParams[$paramNames[$a]] = $tParams[$paramNames[$a]].Identity }
...
#                    { $_ -eq [DataONTAP.C.Types.Cifs.CifsServerConfig] }         { $tParams[$paramNames[$a]] = $tParams[$paramNames[$a]].Identity }
...
function Initialize
{
    ...
    $data.FirstPass = $firstPass.IsPresent
...
function RehydrateCommand
{
    ...
    if(-not [String]::IsNullOrEmpty($commandObject.callee))
    {
        if(-not [String]::IsNullOrEmpty($commandObject.Controller))
        {
            if($Global:cDot.ContainsKey($commandObject.Controller))
            {
                $commandObject.Controller = $Global:cDot[$commandObject.Controller]
            } `
            else
            {
                LogError ("Command object for callee {0} specifies controller object '{1}' not contained in `$Global:cDot." -f @($commandObject.callee, $commandObject.Controller))
            }
        } `
        else
        {
            # Nothing, not a NetApp cmdlet...
        }

        switch ($commandObject.callee)
        {
            "Invoke-NcSnapmirrorUpdate" { <# Nothing #> }
            default
            {
                Write-Host $commandObject.callee
            }
        }
    } `
    else
    {
        LogError "Command object missing callee property."
    }
}
...
function ShutdownCIFSServer
{
    ...
    if($Script:TakeAction)
    {
        ...
        if($data.FirstPass)
        {
            <#  NOTE:  I would make the following block its own function, but that would render it useless since $MyInvocation would represent that function, and not this one.  #>
            $funcParams = @{
                callee = $MyInvocation.MyCommand.Name
            }
            foreach($paramName in @($MyInvocation.MyCommand.Parameters.Keys | Where-Object { $_ -notin @("data", "Verbose", "Debug", "ErrorAction", "WarningAction", "InformationAction", "ErrorVariable", "WarningVariable", "InformationVariable", "OutVariable", "OutBuffer", "PipelineVariable")}))
            {
                $paramValue = (Get-Variable -Name $paramName).Value
                $funcParams.Add($paramName, $paramValue)
            }

            $actionCommand = NewActionSequenceCommand -funcParameters $funcParams
            $data.ActionSequence.Add($actionCommand)
        } `
        else
        {
...
function BreakSnapmirror
{
    ...
    if($data.FirstPass)
    {
        # Just to satisfy later conditionals
        $tSM.MirrorState = "snapmirrored"
    } `
    else
    {
...
function UpdateVolumeEfficiencySettings
{
    ...
    # We'll only need to add an action command for this call of UpdateVolumeEfficiencySettings once, but there are 2 places in this function which could result in a change,
    #   So track if we have already added a command to the action sequence.
    $actionCommandAdded = $false
    $good2Go = $true
        ...
        if(-not $volSISEnabled)
        {
            if($Script:TakeAction)
            {
                ...
                if($data.FirstPass)
                {
                    ...
                    $actionCommand = NewActionSequenceCommand -funcParameters $funcParams
                    $data.ActionSequence.Add($actionCommand)
                    $actionCommandAdded = $true

                    # Create a $volSIS object so we return something the caller can understand.
                    if($null -eq $volSIS)
                    {
                        $volSIS = [DataONTAP.C.Types.Sis.SisStatusInfo]::new()
                    } `
                    else
                    {
                        # Nothing
                    }
                    $volSIS.State = "enabled"
                } `
                else
                {
                    ...
                    if($result.Good2Go)
                    {
                        ...
                        # Again, I can reuse $funcParams...sort of
                        if($funcParams.ContainsKey("callee"))
                        {
                            $funcParams["callee"] = "Get-NcSis"
                        } `
                        else
                        {
                            $funcParams.Add("callee", "Get-NcSis")
                        }
                        $result = ReTryCatch -funcParameters $funcParams
                        if($result.Good2Go)
                        {
        ...
        if($Script:TakeAction)
        {
            ...
            if($volSIS.State -eq "enabled")
            {
                ...
                if($data.FirstPass)
                {
                    ...
                    {
                        # Nothing, we've already created added this call of UpdateVolumeEfficiencySettings to the action sequence
                    }
                } `
                else
                {
    ...
    if($data.Good2Go)
    {
        if($null -ne $sourceCIFSServer)
        {
            if($null -ne $destinationCIFSServer)
            {
                ...
                LogInfo ("Getting computer object for {0} from AD." -f @($sourceCIFSServer.CifsServer)) 1
                $sourceCIFSServerADComputer = GetADComputer -computerName $sourceCIFSServer.CifsServer

                if($null -ne $sourceCIFSServerADComputer)
                {
                    LogInfo ("Received: {0}" -f @($sourceCIFSServerADComputer.DistinguishedName)) 2

                    LogInfo ("Getting computer object for {0} from AD." -f @($destinationCIFSServer.CifsServer)) 1
                    $destCIFSServerADComputer = GetADComputer -computerName $destinationCIFSServer.CifsServer
                    ...
                    if($null -ne $destCIFSServerADComputer)
                    {
                        ...
                        {
                            LogWarning ("{0}'s AD computer object's service principal name list does not appear to have any alias entries!" -f @($sourceCIFSServer.CifsServer))
                            $sourceCIFSServerADComputer.servicePrincipalName.ForEach({
                                LogWarning $_ 1
                            })
                    ...
                    {
                        LogError ("Failed to acquire AD computer object for: {0}" -f @($destinationCIFSServer.CifsServer)) 2
                        $data.Good2Go = $false
                    }
                } `
                else
                {
                    LogError ("Failed to acquire AD computer object for: {0}" -f @($sourceCIFSServer.CifsServer)) 2
                    $data.Good2Go = $false
                }
            } `
            else
        ...
        if($null -ne $destinationCIFSServer)
        {
            ...
            if($null -ne $Script:domainController)
            {
                ...
                if($result.Good2Go -and ($null -ne $result.ReturnValue[0]))
                {
                    ...
                    # If we are running in simulation mode (or running pass 1), use the source CIFS server because the destination CIFS server will not have the right service principal names attached to it.
                    if($Script:TakeAction -or (-not $data.FirstPass))
                    {
                        $computerName = $destinationCIFSServer.CifsServer
                    } `
                    else
                    {
                        $computerName = $sourceCIFSServer.CifsServer
                    }
                    LogInfo ("Getting computer object for {0} from AD." -f @($destinationCIFSServer.CifsServer)) 1
                    ...
                    {
                        $hostNameToAlias = ("{0}.{1}" -f @($destinationCIFSServer.CifsServer, $adDomain.DNSRoot)).ToLower()
                    }
...
function TransferServicePrincipalNamesAndCNAMEs
{
    ...
    if($data.Good2Go)
    {
        TransferServicePrinicipalNames -data $data -sourceCIFSServer $sourceCIFSServer -destinationCIFSServer $destinationCIFSServer
        if($data.Good2Go)
        {
            TransferCNAMERecords -data $data -sourceCIFSServer $sourceCIFSServer -destinationCIFSServer $destinationCIFSServer
        } `
        else
        {
            # Nothing, already displayed a message
    ...
    if($data.Good2Go)
    {
        ...
        if($data.Good2Go)
        {
            ...
            while($a -lt $data.NewSnapmirrors.Count)
            {
                ...
                while($b -lt $data.NewSnapmirrors[$a].Relationships.Count)
                {
                    $snapmirrorUpdated = $snapmirrorBroken = $snapshotPolicyUpdated = $snapmirrorDeleted = $snapmirrorReleased = $false
                    $tmpSM = $null
            ...
            {
                # Step 10: Update service principal names and CNAME records for CIFS
                TransferServicePrincipalNamesAndCNAMEs -data $data -sourceCIFSServer $data.Source.CIFSServer -destinationCIFSServer $data.Destination.CIFSServer
