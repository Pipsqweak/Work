...
function BreakSnapmirror
{
    ...
    if($good2Go)
    {
        if($snapmirrorReady)
        {
            try
            {
                LogInfo ("Breaking snapmirror {0}:{1}:{2}.{3}" -f @($snapmirror.NcController.Name, $snapmirror.Vserver, $snapmirror.DestinationVolume, $Global:simulatedMsg)) 2
                if(-not $Simulated)
                {
                    ...
                    # Invoke-NcSnapmirrorResync -Controller $snapmirror.NcController -DestinationVserver $snapmirror.Vserver -DestinationVolume $snapmirror.DestinationVolume -Confirm:$false -ErrorAction Stop
                }
...
function UpdateSnapshotPolicy
{
    ...
    if(-not [String]::IsNullOrEmpty($sourceVolumeSnapshotPolicyName))
    {
        ...
        if($destinationVolume.VolumeSnapshotAttributes.SnapshotPolicy -notmatch ("{0}`$" -f @($sourceVolumeSnapshotPolicyName)))
        {
            ...
            LogInfo "Getting snapshot policies from the destination cluster which match the source volume snapshot policy" 3
            ...
            if($volSIS.State -eq "enabled")
            {
                try
                {
                    <# Step 5b: Update volume efficiency settings on the destination volume.
                        ...
                        --> vol efficiency modify -vserver LABDR-SMB02 -volume SMDV_vol_LAB_SMB02_vol_SMB_DRTest_01 -policy default -compression true -inline-compression true
                    #>
                    # If we need to change a SIS parameter, we'll flip this.
                    $setSIS = $false
                    $setSISParams = @{
                    ...
                    if($volSIS.IsCompressionEnabledSpecified -and (-not $volSIS.IsCompressionEnabled))
                    {
                        $setSIS = $true
                        $setSISParams.Add("Compression", $true)
                    }
                    else
                    {
                        # Nothing, Compression already enabled.
                    }

                    if($volSIS.IsInlineCompressionEnabledSpecified -and (-not $volSIS.IsInlineCompressionEnabled))
                    {
                        $setSIS = $true
                        $setSISParams.Add("InlineCompression", $true)
                    }
                    else
                    {
                        # Nothing, InlineCompression already enabled.
                    }

                    if($volSIS.Policy -ne "default")
                    {
                        $setSIS = $true
                        $setSISParams.Add("Policy", "default")
                    }
                    else
                    {
                        # Nothing, InlineCompression already enabled.
                    ...
                    if($setSIS)
                    {
                        LogInfo "Setting storage efficiency parameters..." 3
                        $result = Set-NcSis @setSISParams # -Name $volume.Name -Compression $true -Controller $volume.NcController -InlineCompression $true -Policy "default" -VserverContext $volume.VServer -ErrorAction Stop
                    }
                }
                catch
                {
...
{
    $info = "" | Select-Object Source, Destination, Snapmirrors

    $info.Source = "" | Select-Object Volumes, CIFSServer, VServer
    $info.Source.Volumes = [System.Collections.Generic.List[System.Object]]::new()
    $info.Destination = "" | Select-Object Volumes, CIFSServer, VServer
    $info.Destination.Volumes = [System.Collections.Generic.List[System.Object]]::new()
    $info.Snapmirrors = "" | Select-Object SourceVolume, Relationships
    $info.Snapmirrors.Relationships = [System.Collections.Generic.List[System.Object]]::new()
...
if($good2Go)
{
    ...
    if($svmPeersOk)
    {
        LogInfo "GREAT NEWS!!!  All SVM reverse peerings appear to be in-place!!" 1

        # Get all volumes hosted on all ONTAP clusters.  Seems overkill, but this way, we have all the volumes for later use.
        LogInfo "Retrieving all ONTAP volumes from all clusters..."
        try
        {
