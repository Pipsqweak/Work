...
# Here is where I capture the session data...
    ...
    {
        $d = "" | Select-Object CollectionTimeOA, Year, Month, Day, Hour, Minute, NumericDayOfWeek, SortableDayAndTime, DC, Site, Count
        $d.CollectionTimeOA = $collectionTime.ToOADate()
        $d.Year = $collectionTime.Year
        $d.Month = $collectionTime.Month
        $d.Day = $collectionTime.Day
        $d.Hour = $collectionTime.Hour
        $d.Minute = $collectionTime.Minute
        $d.NumericDayOfWeek = [int] $collectionTime.DayOfWeek
        $d.SortableDayAndTime = "{0} {1} {2:D2}:{3:D2}" -f @($d.NumericDayOfWeek, $collectionTime.DayOfWeek,$d.Hour, $d.Minute)
    ...
    $cifsData | Export-Excel -Append -Path "E:\Data\Stats\CifsSessions2.xlsx"
}
catch {
