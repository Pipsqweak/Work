<#
    Utilizes a Global variable: $Global::isError.  If [Log]::Error is called, $Global::isError is set so other code can check to see if
    an error has been logged.  In the past, I've used this capability to stop additional processing.
#>
    ...
    class Log
    {
        ...
        hidden [void] Output([LogLevel] $msgLevel, [String] $message, [Boolean] $mustOutput)
        {
            if(-not [String]::IsNullOrEmpty($message))
            {
                ...
                # Only log the message if the logging level is GE to the message level.
        ...
        static [void] Init([String] $logRootPath, [String] $logName, [System.Int32] $maxLogAge, [System.Int32] $lineCache = 1, [LogLevel] $loggingLevel = [LogLevel]::INFO)
        {
            if(-not [String]::IsNullOrEmpty($logName))
            {
                ...
                {
                    # Make sure the complete path is available for logging.
                    New-Item -Path $logRootPath -ItemType Directory -Force | Out-Null
                    ...
                    if (-not $Global:isError)
                    {
                        if(Test-Path -LiteralPath $logRootPath)
                        {
                            ...
                            $oldLogs = @(Get-ChildItem -Path $logRootPath | Where-Object { (-not $_.PSIsContainer) -and ($_.BaseName.Contains($logName)) -and ($_.CreationTime -le [DateTime]::Now.AddDays(-1 * $maxLogAge)) })
                        ...
                        $logFileName = ("{0}\{1}-{2}.log" -f @($logRootPath, $logName, $dtNow)).Replace("\\","\")

                        # If, after replace all the \\ with \, there is a leading \, add another \ so the UNC is correct.
        ...
        static [void] Separator() { [Log]::Must([String]::new('=', 100)) }
        <#
        ...
        static [void] Error([System.String[]] $messageStrings)
        {
