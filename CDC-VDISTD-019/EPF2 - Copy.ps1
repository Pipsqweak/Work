...
function GetPublicFolderByEntryID
{
    ...
    if($null -ne $Script:OutlookNamespace)
    {
        ...
        if($Script:ReturnObject.Good2Go)
        {
            ...
            {
                LogError ("Failed to get public folder with entry ID: {0} from Outlook." -f @($Script:ReturnObject.PublicFolder.EntryID))
                $publicFolder = $null
                $Script:ReturnObject.Good2Go = $false
            }
        } `
...
function GetPFPWFolderData
{
    ...
    if($null -ne $Script:topPublicFolder)
    {
        ...
        if($null -ne $retVal.PublicFolder)
        {
            ...
            if(($retVal.PublicFolder.DefaultItemType -as [Microsoft.Office.Interop.Outlook.OlItemType]) -eq [Microsoft.Office.Interop.Outlook.OlItemType]::olAppointmentItem)
            {
