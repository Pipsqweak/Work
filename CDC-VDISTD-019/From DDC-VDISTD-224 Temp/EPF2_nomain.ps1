...
function ExportPublicFolderItemsAsFiles
{
    ...
    if ($publicFolder.EntryID.SubString(44) -eq "000000009598E9A20E04F041B38518182890A2D50100A3DF33FCC9BD1644BD14FCA330AA97850000281D4B490000".SubString(44))
    {
...
function CheckPF2PWFolder
{
    ...
    if($null -ne $pfPWData)
    {
        ...
        {
            $cData.Identity = $pfPWData.PublicFolder.FolderPath.Replace($Script:topPublicFolder.FolderPath, "").Trim([System.IO.Path]::DirectorySeparatorChar)
            ...
            if($cData.PFDocumentCount -gt 0)
            {
                if($null -ne $pfPWData.ProjectWiseFolder)
                {
                    if($null -ne $pfPWData.PWFolderDocumentSize)
                    {
                        ...
                        {
                            LogInfo ("{0} ({1}) and PW document counts DO NOT match.  PF: {2}, PW: {3}" -f @($cData.Identity, $cData.PFType, $pfPWData.PFDocumentCount, $pfPWData.PWFolderDocumentSize.NumberOfContainedDocuments))
                        }
                    } `
                    else # NOT ($null -ne $pfPWData.PWFolderDocumentSize)
                    {
                        LogInfo ("No document size data available for ProjectWise folder: {0}" -f @($cData.Identity))
                    }
                } `
                else # NOT ($null -ne $pfPWData.ProjectWiseFolder)
                {
                    ...
                    {
                        LogInfo ("No ProjectWise folder found for PW (test) path: {0}" -f @($pfPWData.ProjectWiseTestFolderPath))
                    } `
                    else # NOT (-not [String]::IsNullOrEmpty($pfPWData.ProjectWiseTestFolderPath))
                    {
                        LogInfo ("No ProjectWise folder found for PF path: {0} ({1})" -f @($cData.Identity, $cData.PFType))
                    }
                }
            } `
        ...
        {
            LogInfo ("No public folder found for entry ID: {0}" -f @($cData.EntryID))
        }
    } `
...
function ProcessPFList
{
    ...
    if($Script:ReturnObject.Good2Go)
    {
        ...
        while($pfListIdx -lt $pfList.Length)
        {
            ...
            {
                $status = "{0,7:P2} Complete | Compared: {1} | Remaining: {2}" -f @($percentComplete, $pfListIdx, ($pfList.Length - $pfListIdx))
                Write-Progress -Id 0 -Activity "Comparing PFs to PW..." -Status $status -PercentComplete ($percentComplete * 100.0)
                CheckPF2PWFolder -entryID $pfList[$pfListIdx].EntryID
            }
            $pfListIdx++
        }
