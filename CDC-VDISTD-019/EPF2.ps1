...
if($CheckPF2PW.IsPresent)
{
    $Script:LogFileNamePrefix = "ComparePF2PW"
} `
else # NOT ($CheckPF2PW.IsPresent)
{
    if(-not [String]::IsNullOrEmpty($Script:publicFolderEntryID))
    {
        $Script:LogFileNamePrefix = "ExportPF-{0}" -f @($Script:publicFolderEntryID)
    } `
    else # NOT (-not [String]::IsNullOrEmpty($Script:publicFolderEntryID))
    {
...
# Was a calendar exported?
$Script:ExportedAsICS = $false
...
# Add all the unicode characters I've found along the way.
$Script:BadFilenameChars += [char]0xD83D
$Script:BadFilenameChars += [char]0xDD34
    ...
    [int] Compare([System.Object] $pfObj1, [System.Object] $pfObj2) {
        ...
        else
        {
            ...
            else
            {
                ...
                if(($null -eq $pfObj1.SortTime) -and ($null -eq $pfObj2.SortTime))
                {
                    $retVal = 0
                } `
                elseif($null -eq $pfObj1.SortTime)
                {
                    $retVal = -1
                } `
                elseif($null -eq $pfObj2.SortTime)
                {
                    $retVal = 1;
                } `
                else
                {
                    $retVal = $pfObj1.SortTime.CompareTo($pfObj2.SortTime)
                } `
            }
        }
...
function ImportInteropDLL
{
    try
    {
        ...
        try
        {
            $interop_assemply_file = @(Get-ChildItem -Recurse  "Microsoft.Office.Interop.Outlook.dll" -ErrorAction Stop)
            if($interop_assemply_file.Length -gt 0)
            {
                try
                {
                    Set-Location -Path $interop_assemply_file[0].Directory -ErrorAction Stop
                    ...
                    try
                    {
                        ...
                        try
                        {
                            $fileVersionInfo = [System.Diagnostics.FileVersionInfo]::GetVersionInfo($interop_assemply_file[0].FullName)
                            LogInfo ("Loaded {0} version: {1}" -f @($interop_assemply_file[0].Name, $fileVersionInfo.ProductVersion))
                        }
                        catch
                        {
                            LogInfo ("Loaded {0}" -f @($interop_assemply_file[0].Name))
                        }

                        try
                        {
                            Set-Location -Path $original_pwd -ErrorAction Stop
                        }
                        catch
                        {
                            # Nothing... script should run fine...just trapping the exception.
                        }
                    }
                    catch
                    {
                        LogException "Failed to load Microsoft.Office.Interop.Outlook assembly"
                        $Script:ReturnObject.Good2Go = $false
                    }
                }
                catch
                {
                    LogException ("Failed to set location to: {0}" -f @($interop_assemply_file[0].Directory))
                    $Script:ReturnObject.Good2Go = $false
                }
            } `
            else # NOT ($interop_assemply_file.Length -gt 0)
            {
                LogError "Failed to locate Microsoft.Office.Interop.Outlook.dll"
                $Script:ReturnObject.Good2Go = $false
            }
        }
        catch
        {
            LogException "Failed to locate Microsoft.Office.Interop.Outlook.dll"
            $Script:ReturnObject.Good2Go = $false
        }
    }
    catch
    {
        LogError "Failed to set location to system assembly directory."
        $Script:ReturnObject.Good2Go = $false
    }
}
...
function SetEntryIDPrefix
{
    ...
    else # NOT ($null -ne $Script:OutlookNamespace)
    {
        LogError "Not attached to Outlook in SetEntryIDPrefix."
        $Script:ReturnObject.Good2Go = $false
    }
}
...
function ConnectToProjectWise
{
    ...
    if(-not $alreadyConnectedToPW)
    {
        if(-not $Script:LoggedPWData)
        {
            ...
            $Script:LoggedPWData = $true
        } `
        else # NOT (-not $Script:LoggedPWData)
        {
            # Nothing.
        }
    ...
    if($Script:ReturnObject.Good2Go)
    {
        if($null -eq $Script:ProjectWiseBaseFolder)
        {
        ...
        else # NOT ($null -eq $Script:ProjectWiseBaseFolder)
        {
            # Nothing, already set $Script:ProjectWiseBaseFolder
        }
    } `
...
function InitializeExporter
{
    if(-not $Script:IsInitialized)
    {
        ...
        $Script:IsInitialized = $Script:ReturnObject.Good2Go
    } `
    else # NOT (-not $Script:IsInitialized)
    {
...
function GetItemFromOutlookByEntryID
{
    ...
    if(-not [String]::IsNullOrEmpty($entryID))
    {
        ...
        do
        {
            ...
            <#
                Try to get an item from Outlook by EntryID...
                    and .... then try to read the item's .EntryID...

                If either of these actions result in an exception, then we'll retry ... if we haven't exceeded the retry count.
            #>

            try
            {
                # Reset the variables just to make sure...
                $item = $null
            ...
            if(($null -eq $item) -or [String]::IsNullOrEmpty($itemEntryID))
            {
                if ($retries -eq $Script:MaxItemGetRetries)
                {
                    LogError ("Failed to get an item with an entry ID using entryID = [{0}]) after {1} retries." -f @($entryID, $Script:MaxItemGetRetries))

                    # Reset $item to $null so the caller knows it's not good.
                    $item = $null
                } `
                else # NOT ($retries -eq $Script:MaxItemGetRetries)
                {
                    Start-Sleep -Milliseconds $Script:ItemGetDelay
                }
            } `
            else # NOT (($null -eq $item) -or [String]::IsNullOrEmpty($itemEntryID))
            {
                # Nothing, got an item with an entryID...
            }
        }  while((($null -eq $item) -or [String]::IsNullOrEmpty($itemEntryID)) -and ($retries -lt $Script:MaxItemGetRetries))
    } `
    else # NOT (-not [String]::IsNullOrEmpty($entryID))
    {
...
function NewPublicFolderObject
{
    $pfObj = [PSCustomObject]@{
        PublicFolderPath = [String]::Empty
        EntryID = [String]::Empty
        Status = [String]::Empty
...
function GetRowValue
{
    ...
    if ($null -ne $row)
    {
        if (-not [String]::IsNullOrEmpty($colName))
        {
            ...
            {
                LogError ("Failed to get value for {0} in GetRowValue." -f @($colName))
                $Script:ReturnObject.Good2Go = $false
            }
        } `
        else # NOT ([String]::IsNullOrEmpty($colName))
        {
            LogError ("Missing column name in GetRowValue")
            $Script:ReturnObject.Good2Go = $false
        }
    } `
    else # NOT ($null -ne $row)
    {
        LogError ("Null row in GetRowValue")
        $Script:ReturnObject.Good2Go = $false
    }
...
function NewPublicFolderObjectFromRow
{
    [CmdletBinding()]
    param
    (
    ...
    if($null -ne $row)
    {
        $pfObj = NewPublicFolderObject
        if ($Script:ReturnObject.Good2Go)
        {
            $pfObj.PublicFolderPath = $Script:ExportedPublicFolderPath
            $pfObj.TempFileIndex = $fileIndex
            $pfObj.TempFileName = "{0}\File_{1:D5}.msg" -f @($Script:WorkingFolder, $pfObj.TempFileIndex)

            $pfObj.EntryID = GetRowValue -row $row -colName "EntryID"
            if ($Script:ReturnObject.Good2Go)
            {
                $pfObj.LastModificationTime = GetRowValue -row $row -colName "LastModificationTime"
                if ($Script:ReturnObject.Good2Go)
                {
                    $pfObj.CreationTime = GetRowValue -row $row -colName "CreationTime"
                    if ($Script:ReturnObject.Good2Go)
                    {
                        $pfObj.Subject = GetRowValue -row $row -colName "Subject"
                        if ($Script:ReturnObject.Good2Go)
                        {
                            ...
                            $pfObj.SortTime = $pfObj.CreationTime

                            # If the item has no creation time (WTH!!) then use LastModificationTime
                            ...
                            $msgClass = GetRowValue -row $row -colName "MessageClass"
                            if ($Script:ReturnObject.Good2Go)
                            {
                            ...
                            {
                                LogError ("Failed to get row message class in NewPublicFolderObjectFromRow")
                            }
                        } `
                        else # NOT ($Script:ReturnObject.Good2Go)
                        {
                            LogError ("Failed to set pfObj.Subject in NewPublicFolderObjectFromRow")
                        }
                    } `
                    else # NOT ($Script:ReturnObject.Good2Go)
                    {
                        LogError ("Failed to set pfObj.CreationTime in NewPublicFolderObjectFromRow")
                    }
                } `
                else # NOT ($Script:ReturnObject.Good2Go)
                {
                    LogError ("Failed to set pfObj.LastModificationTime in NewPublicFolderObjectFromRow")
                }
            } `
            else # NOT ($Script:ReturnObject.Good2Go)
            {
                LogError ("Failed to set pfObj.EntryID in NewPublicFolderObjectFromRow")
            }
        } `
        else # NOT ($Script:ReturnObject.Good2Go)
        {
    ...
    {
        LogError "Null row in NewConversationObjectFromRow."
    }
        ...
        if($null -ne $item)
        {
            ...
            try
            {
                ...
                <#
                    ...
                    $item = GetItemFromOutlookByEntryID -entryID $row["EntryID"]

                    *********************************    READ ********************

                    $item.EntryID WILL NOT BE SAME AS $row["EntryID"]

                    I do not know why, it just isn't...
                #>

                # See note above... update .EntryID
                $pfObj.EntryID = $itemEntryID

                # Yes, this seems redundant, but we might have set .EntryID prior to this function, so check it...
                if ($pfObj.EntryID -eq $itemEntryID)
                {
                    ...
                    switch($item.MessageClass)
                    {
                        ...
                        "IPM.Appointment" {
                            $pfObj.SenderName = $item.Organizer
                            break
                        }
                    }
                ...
                else # NOT ($pfObj -eq $itemEntryID)
                {
                    LogError ("Item/pfObj mismatch.  Item: {0}, PFObj: {1}" -f @($itemEntryID, $pfObj.EntryID))
                }
            }
            catch
            {
                LogException ("Failed to get entry ID from item in UpdatePublicFolderObjectFromItem.")
                $Script:ReturnObject.Good2Go = $false
            }
        } `
        else
        {
            LogError "Null item in UpdatePublicFolderObjectFromItem."
            $Script:ReturnObject.Good2Go = $false
        }
    } `
...
function NewPublicFolderObjectFromItem
{
    ...
    {
        $pfObj = NewPublicFolderObject
        UpdatePublicFolderObjectFromItem -pfObj $pfObj -item $item
        $pfObj.TempFileIndex = $fileIndex
        $pfObj.TempFileName = "{0}\File_{1:D5}.msg" -f @($Script:WorkingFolder, $pfObj.TempFileIndex)
        $pfObj.SenderName = $item.SenderName
...
function SavePublicFolderItemsToLocalFolderUsingEntryIDList
{
    [CmdletBinding()]
    param
    (
    ...
    try
    {
        $entryIDs = @(Get-Content -Path $entryIDListFile -ErrorAction Stop)
    }
    catch
    {
        LogError ("Failed to read item entry IDs from: {0}" -f @($entryIDListFile))
        $Script:ReturnObject.Good2Go = $false
    }
    ...
    if($Script:ReturnObject.Good2Go)
    {
        ...
        $rowNumber = 0
        while($rowNumber -lt $entryIDs.Length)
        {
            ...
            if(-not [String]::IsNullOrEmpty($entryID))
            {
                ...
                if($null -ne $item)
                {
                    ...
                    {
                        LogError ("Item with null .EntryID returned from GetItemFromOutlookByEntryID for public folder {0} [{1}] and item entryID: {2}" -f @($Script:ExportedPublicFolderPath, $publicFolder.EntryID, $entryID))
                        $Script:ReturnObject.Good2Go = $false
                    }

                    if(-not [String]::IsNullOrEmpty($itemEntryID))
                    {
                        ...
                        if($null -ne $pfObj)
                        {
                            ...
                            #   NOTE:  $pfObj will always have a .ConversationID since NewPublicFolderObjectFromItem ensures it is set.
                            ...
                            # Add $pfObj to $Script:ItemsByConversation[$pfObj.ConversationID] in .ConversationIndex order...
                        ...
                        {
                            LogError ("Null public folder object returned from NewPublicFolderObjectFromItem for entry ID: {0}" -f @($itemEntryID))
                            $Script:ReturnObject.Good2Go = $false
                        }
                    } `
...
function GetPublicFolderItemDataFromTable
{
    [CmdletBinding()]
    param
    (
    ...
    if ($null -ne $publicFolder)
    {
        ...
        $itemEntryIDs = [System.Collections.Generic.List[System.String]]::new()
        $table = $null
        ...
        {
            LogError ("No table returned from .GetTable().")
            $Script:ReturnObject.Good2Go = $false
        }
        ...
        if ($null -ne $table)
        {
            ...
            {
                $elapsedTicks = $sw.ElapsedTicks
                $ticksPerItem = $elapsedTicks / ($rowNumber + 1)
                $totalETATicks = $ticksPerItem * $rowCount
                $remainingETATicks = $totalETATicks - $elapsedTicks
                $etaTS = [TimeSpan]::new($remainingETATicks)
                $etaDT = [DateTime]::Now.Add($etaTS)

                $percentComplete = ($rowNumber + 1) / $rowCount
                $status = "{0} of {1} | {2,7:P2} Complete | Elapsed: {3} | Est Remaining: {4} | ETC: {5}" -f @(($rowNumber + 1), $rowCount, $percentComplete, $sw.Elapsed.ToString(), $etaTS.ToString(), $etaDT.ToString("HH:mm:ss.fffff"))
                Write-Progress -Id 1 -Activity $activity -Status $status -PercentComplete ($percentComplete * 100.0)
                ...
                try
                {
                    ...
                    if($null -ne $row)
                    {
                        $entryID = GetRowValue -row $row -colName "EntryID"
                        if ($Script:ReturnObject.Good2Go)
                        {
                            if(-not [String]::IsNullOrEmpty($entryID))
                            {
                                $i = $itemEntryIDs.BinarySearch($entryID)
                                if($i -lt 0)
                                {
                                    $itemEntryIDs.Insert(-bnot $i, $entryID)
                                    $pfObj = NewPublicFolderObjectFromRow -row $row -fileIndex $rowNumber
                                    $Script:PublicFolderItems.Add($pfObj)
                                }
                                else
                                {
                                    LogError ("Duplicate entryID: {0} at row number {1}" -f @($entryID, $rowNumber))
                                    $Script:ReturnObject.Good2Go = $false
                                }
                            }
                            else
                            {
                                LogError ("Empty entry ID at row number: {0}" -f @($rowNumber))
                                $Script:ReturnObject.Good2Go = $false
                            }
                        } `
                        else # NOT ($Script:ReturnObject.Good2Go)
                        {
                            LogError ("Failed to get entry ID from row in GetPublicFolderItemDataFromTable.")
                        }
                    }
                    else
                    {
                        LogError ("Null row at row number: {0}" -f @($rowNumber))
                        $Script:ReturnObject.Good2Go = $false
                    }
                }
                catch
                {
                    LogError ("Failed to GetNextRow at row number: {0}" -f @($rowNumber))
                    $Script:ReturnObject.Good2Go = $false
                }
    ...
    {
        LogError ("Null public folder in GetPublicFolderItemDataFromTable.")
        $Script:ReturnObject.Good2Go = $false
    }
}

function SaveLargePublicFolderItemsToLocalFolder
{
    [CmdletBinding()]
    param
    (
    ...
    if ($null -ne $publicFolder)
    {
        GetPublicFolderItemDataFromTable -publicFolder $publicFolder
        SavePFObjectsAndImportResults
        ...
        if ($Script:ReturnObject.Good2Go)
        {
            $Script:ReturnObject.ExportedItems.Count = 0
            $Script:ReturnObject.ExportedItems.Size = 0
            $rowCount = $Script:PublicFolderItems.Count
            $sw = [System.Diagnostics.Stopwatch]::new()
            $sw.Start()
            $activity = "Saving {0} items to local folder..." -f @($rowCount)
            ...
            while($a -lt $Script:PublicFolderItems.Count)
            {
                ...
                $totalETATicks = $ticksPerItem * $Script:PublicFolderItems.Count
                $remainingETATicks = $totalETATicks - $elapsedTicks
                $etaTS = [TimeSpan]::new($remainingETATicks)
                $etaDT = [DateTime]::Now.Add($etaTS)
                $percentComplete = ($a + 1) / $rowCount
                $status = "{0} of {1} | {2,7:P2} Complete | Elapsed: {3} | Est Remaining: {4} | ETC: {5}" -f @(($a + 1), $rowCount, $percentComplete, $sw.Elapsed.ToString(), $etaTS.ToString(), $etaDT.ToString("HH:mm:ss.fffff"))
                Write-Progress -Id 1 -Activity $activity -Status $status -PercentComplete ($percentComplete * 100.0)

                $item = GetItemFromOutlookByEntryID -entryID $pfObj.EntryID
    ...
    {
        LogError ("Null public folder in SaveLargePublicFolderItemsToLocalFolder.")
        $Script:ReturnObject.Good2Go = $false
    }
}
...
function ExportPublicFolderItemsAsFiles
{
    ...
    if ($publicFolder.EntryID.SubString(44) -eq "xxx000000009598E9A20E04F041B38518182890A2D50100A3DF33FCC9BD1644BD14FCA330AA97850000281D4B490000".SubString(44))
    {
        SavePublicFolderItemsToLocalFolderUsingEntryIDList -publicFolder $publicFolder -entryIDListFile "C:\TEMP\T2\Lists\000000009598E9A20E04F041B38518182890A2D50100A3DF33FCC9BD1644BD14FCA330AA97850000281D4B490000Test.txt"
    } `
    else # NOT ($publicFolder.EntryID -eq "000000009598E9A20E04F041B38518182890A2D50100A3DF33FCC9BD1644BD14FCA330AA97850000281D4B490000")
    {
        SaveLargePublicFolderItemsToLocalFolder -publicFolder $publicFolder
    }
...
function GetPublicFolderByEntryID
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true, Position = 0)]
    ...
    if($null -ne $Script:OutlookNamespace)
    {
        $entryID = FixEntryID -entryIDtoFix $entryID
        if($Script:ReturnObject.Good2Go)
        {
            try
            {
                $publicFolder = $Script:OutlookNamespace.GetFolderFromID($entryID)
            }
            catch
            {
                LogError ("Failed to get public folder with entry ID: {0} from Outlook." -f @($entryID))
                $publicFolder = $null
                $Script:ReturnObject.Good2Go = $false
            }
        } `
    ...
    {
        LogError "Not attached to Outlook."
        $Script:ReturnObject.Good2Go = $false
    }
...
function ExportPublicFolderToDisk
{
    ...
    if($Script:ReturnObject.Good2Go)
    {
        if($null -ne $publicFolder)
        {
            $Script:ReturnObject.PublicFolder.EntryID = FixEntryID -entryIDtoFix $entryID
            $Script:ReturnObject.ExportedItems.Count = 0
            ...
            if($publicFolder.FolderPath -notmatch "DUMPSTER_ROOT")
            {
                ...
                if($Script:ReturnObject.PublicFolder.ItemCount -gt 0)
                {
                    ...
                    LogInfo ("`tItem count: {0}" -f @($Script:ReturnObject.PublicFolder.ItemCount))
        ...
        {
            LogError ("No public folder found using entry ID: {0}" -f @($entryID))
            $Script:ReturnObject.Good2Go = $false
        }
    } `
    else # NOT ($Script:ReturnObject.Good2Go)
...
function VerifyCreatePWPath
{
    ...
    if($true -or -not [String]::IsNullOrEmpty($path))
    {
        if(-not $Quiet.IsPresent)
        {
            LogInfo ("Verifying ProjectWise path: {0}" -f @($path))
        } `
        else # NOT (-not $Quiet.IsPresent)
        {
            # Nothing.
        }
        ...
        if($haveLock)
        {
            ...
            if($subFolders.Length -gt 0)
            {
                ...
                $subFolderIdx = 0
                ConnectToProjectWise    # for tasks which run a long time, this is necessary.
                if($Script:ReturnObject.Good2Go)
                {
                    do
                    {
    ...
    return @($pwFolderExists, $pwFolder)
}
...
function RemoveUselessFolder
{
    ...
    {
        LogWarning ("'Useless' folder: {0} is not empty" -f @($path))
    }
}
                ...
                $importedFiles.ForEach({
                    ...
                    if($null -ne $filesInImportFolder)
                    {
                        ...
                        if($null -ne $importedFile)
                        {
                            ...
                            else # NOT (-not $Script:ExportedAsICS)
                            {
                                # Nothing.
                            }
...
{
    ExportPublicFolderToDisk -entryID $Script:publicFolderEntryID
...
function ExportPublicFolderByEntryID
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true, Position = 0)]
    ...
    if(-not (PublicFolderHasBeenExported -entryID $entryID))
    {
        if(-not (PublicFolderActivelyBeingExported -entryID $entryID))
        {
        ...
        $Script:PFEntryID = $entryID
        # Reset $Script:ReturnObject back to factory defaults
        ResetScriptObjects -PFEntryID $entryID

        if($Script:ReturnObject.Good2Go)
        {
            # Proceed with transferring items from public folders to ProjectWise...
            LogInfo ("Starting export of public folder entry ID: {0}" -f @($entryID))
    ...
    {
        LogInfo ("Public folder with entry ID: {0} has already been successfully exported." -f @($entryID))
    }
}
...
function StartNewExporter
{
    ...
    $cmdArgs = @(
        ...
        "-publicFolderEntryID", (Quoted $publicFolderIdentity.EntryID),
        "-PWServerFQDN", (Quoted $Script:PWServerFQDN),
        "-PWDatasourceName", (Quoted $Script:PWDatasourceName),
        "-PWUserName", (Quoted $Script:PWUserName),
        "-PWEncryptedUserPassword", (Quoted $Script:PWEncryptedUserPassword),
        "-ProjectWiseBaseFolderName", (Quoted $Script:ProjectWiseBaseFolderName),
        "-LogFolder", (Quoted $Script:LogFolder),
        "-BaseWorkingFolder", (Quoted $Script:BaseWorkingFolder)
    )

    Start-Process -WindowStyle Normal powershell.exe $cmdArgs -Wait
}
...
function GetPFPWFolderData
{
    ...
    $retVal = [PSCustomObject]@{
        PublicFolder = $null
        ProjectWiseTestFolderPath = $null
        PFDocumentCount = 0       # adjust if the folder is a calendar
        ProjectWiseFolder = $null
        PWFolderDocumentSize = $null
        DocCountMatches = $false
    }

    if($null -ne $Script:topPublicFolder)
    {
        $retVal.PublicFolder = GetPublicFolderByEntryID -entryID $entryID
        if($null -ne $retVal.PublicFolder)
        {
            $retval.PFDocumentCount = $retVal.PublicFolder.Items.Count
            if(($retval.PFDocumentCount -gt 1) -and (($retVal.PublicFolder.DefaultItemType -as [Microsoft.Office.Interop.Outlook.OlItemType]) -eq [Microsoft.Office.Interop.Outlook.OlItemType]::olAppointmentItem))
            {
                $retval.PFDocumentCount = 1    # calendars are always exported as a single item.
            } `
            else
            {
                # Nothing
            ...
            if($retVal.PFDocumentCount -gt 0)
            {
                $retVal.ProjectWiseTestFolderPath = FixFileOrPath -fileOrPath $retVal.PublicFolder.FolderPath.Replace($Script:topPublicFolder.FolderPath, "").Trim([System.IO.Path]::DirectorySeparatorChar)
                $pwFolderExists, $retVal.ProjectWiseFolder = VerifyCreatePWPath -path $retVal.ProjectWiseTestFolderPath -Quiet

                if($pwFolderExists)
                {
                    try
                    {
                        $retVal.PWFolderDocumentSize = Get-PWFolderDocumentSize -InputFolder $retVal.ProjectWiseFolder
                    }
                    catch
                    {
                        $retVal.PWFolderDocumentSize = $null
                    }

                    if($null -ne $retVal.PWFolderDocumentSize)
                    {
                        $retval.DocCountMatches = $retval.PFDocumentCount -eq $retVal.PWFolderDocumentSize.NumberOfContainedDocuments
                    } `
                    else
                    {
                        # Nothing
            ...
            else # NOT ($retVal.PFDocumentCount -gt 0)
            {
                # Nothing.
            }
        } `
        ...
        {
            # Nothing Write-Host ("No public folder found with entry ID: {0}" -f @($Script:publicFolderEntryID))
        }
    } `
    else
    {
        # Error no topPublicFolder.
    }

    return @( ,$retval)
}

function CheckPF2PWFolder
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory = $true, Position = 0)]
    ...
    $cData = [PSCustomObject]@{
        ...
        PFType = [String]::Empty
        PFDocumentCount = 0
        PWFolderDocumentCount = 0
    }

    $pfPWData = GetPFPWFolderData -entryID $cData.EntryID
    if($null -ne $pfPWData)
    {
        if($null -ne $pfPWData.PublicFolder)
        {
            $cData.PFType = ($pfPWData.PublicFolder.DefaultItemType -as [Microsoft.Office.Interop.Outlook.OlItemType]).ToString()
            $cData.PFDocumentCount = $pfPWData.PFDocumentCount

            if($cData.PFDocumentCount -gt 0)
            {
                if($null -ne $pfPWData.ProjectWiseFolder)
                {
                    if($null -ne $pfPWData.PWFolderDocumentSize)
                    {
                        $cData.PWFolderDocumentCount = $pfPWData.PWFolderDocumentSize.NumberOfContainedDocuments
                        if($pfPWData.DocCountMatches)
                        {
                            LogInfo ("{0} ({1}) and PW document counts match: {2}" -f @($cData.Identity, $cData.PFType, $pfPWData.PFDocumentCount))
                        } `
                        else # NOT ($pfPWData.DocCountMatches)
                        {
                            LogWarning ("{0} ({1}) and PW document counts DO NOT match.  PF: {2}, PW: {3}" -f @($cData.Identity, $cData.PFType, $pfPWData.PFDocumentCount, $pfPWData.PWFolderDocumentSize.NumberOfContainedDocuments))
                        }
                    } `
                    else # NOT ($null -ne $pfPWData.PWFolderDocumentSize)
                    {
                        LogWarning ("No document size data available for ProjectWise folder: {0}" -f @($cData.Identity))
                    }
                } `
                else # NOT ($null -ne $pfPWData.ProjectWiseFolder)
                {
                    if(-not [String]::IsNullOrEmpty($pfPWData.ProjectWiseTestFolderPath))
                    {
                        LogWarning ("No ProjectWise folder found for PW (test) path: {0}" -f @($pfPWData.ProjectWiseTestFolderPath))
                    } `
                    else # NOT (-not [String]::IsNullOrEmpty($pfPWData.ProjectWiseTestFolderPath))
                    {
                        LogWarning ("No ProjectWise folder found for PF path: {0} ({1})" -f @($cData.Identity, $cData.PFType))
                    }
                }
            } `
            else # NOT ($pfPWData.PFDocumentCount -gt 0)
            {
                LogInfo ("{0} ({1}) is empty." -f @($cData.Identity, $cData.PFType))
            }
        } `
        else # NOT ($null -ne $pfPWData.PublicFolder)
        {
            LogError ("No public folder found for entry ID: {0}" -f @($cData.EntryID))
        }
    } `
    else # NOT ($null -ne $pfPWData)
    {
        LogError ("Null data returned from GetPFPWFolderData.")
        $Script.ReturnValue.Good2Go = $false
    }

    if($null -eq $Script:comparisonData)
    {
        $Script:comparisonData = [System.Collections.Generic.List[System.Object]]::new()
    } `
    else # NOT ($null -eq $Script:comparisonData)
    {
        # Nothing.
    }

    $cData | Export-CSV -Delimiter "`t" -Path $redoFile -Append -NoTypeInformation
    $Script:comparisonData.Add($cData)
}
...
function ProcessPFEntryID
{
    ...
    if($Script:CheckPF2PW.IsPresent)
    {
        CheckPF2PWFolder -entryID $Script:publicFolderEntryID
    } `
    else
    {
        ExportPublicFolderByEntryID -entryID $Script:publicFolderEntryID
    }
...
function ProcessPFList
{
    ...
    if($Script:ReturnObject.Good2Go)
    {
        ...
        if(-not $Script:CheckPF2PW.IsPresent)
        {
            $activity = "Xfering PFs to PW..."
        } `
        else # NOT (-not $Script:CheckPF2PW.IsPresent)
        {
            $fi = [System.IO.FileInfo]::new($Script:publicFolderListFile)
            $redoFile = "{0}\{1}_redo.csv" -f @($fi.Directory, $fi.BaseName)
            $activity = "Comparing PFs to PW..."
        }
        ...
        {
            $elapsedTicks = $sw.ElapsedTicks
            $ticksPerItem = $elapsedTicks / ($pfListIdx + 1)
            $totalETATicks = $ticksPerItem * $pfList.Length
            $remainingETATicks = $totalETATicks - $elapsedTicks
            $etaTS = [TimeSpan]::new($remainingETATicks)
            $etaDT = [DateTime]::Now.Add($etaTS)
            $percentComplete = ($pfListIdx / $pfList.Length)
            $status = "{0} of {1} | {2,7:P2} Complete | Elapsed: {3} | Est Remaining: {4} | ETC: {5}" -f @(($pfListIdx + 1), $pfList.Length, $percentComplete, $sw.Elapsed.ToString(), $etaTS.ToString(), $etaDT.ToString("HH:mm:ss.fffff"))
            Write-Progress -Id 0 -Activity $activity -Status $status -PercentComplete ($percentComplete * 100.0)

            if(-not $Script:CheckPF2PW.IsPresent)
            {
                StartNewExporter -publicFolderIdentity $pfList[$pfListIdx]
                # ExportPublicFolder -publicFolderIdentity $pfList[$pfListIdx]
            } `
            else # NOT (-not $Script:CheckPF2PW.IsPresent)
            {
                CheckPF2PWFolder -entryID $pfList[$pfListIdx].EntryID -Identity $pfList[$pfListIdx].Identity -redoFile $redoFile
            }
...
<#
        if($Script:CheckPF2PW.IsPresent -and ($null -ne $Script:comparisonData))
        {
            $fi = [System.IO.FileInfo]::new($Script:publicFolderListFile)
            $redoFile = "{0}\{1}_redo.csv" -f @($fi.Directory, $fi.BaseName)
            $a = 0
            while($a -lt $Script:comparisonData.Count)
            {
                $pfListItem = $pfList | Where-Object { $_.EntryID -eq $Script:comparisonData[$a].EntryID }
                if($null -ne $pfListItem)
                {
                    $Script:comparisonData[$a].Identity = $pfListItem.Identity
                } `
                else # NOT ($null -ne $pfListItem)
                {
                    # Nothing.
                }
                $a++
            }
            $Script:comparisonData | Export-CSV -Delimiter "`t" -NoTypeInformation -Path $redoFile
        } `
        else # NOT ($Script:CheckPF2PW.IsPresent -and ($null -ne $Script:comparisonData))
        {
            # Nothing.
        }
#>
