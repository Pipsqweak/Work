    ...
    try
    {
        ...
        if ($xclaritySvrs.Length -gt 0)
        {
            ...
            while($l -lt $xclaritySvrs.Length)
            {
                ...
                $serverDataPoint.SetSerialNumber($xclaritySvr.SerialNumber)
                $serverDataPoint.SetModel($xclaritySvr.ProductName)

                $xServerFW = @($xclaritySvr.Firmwares | Where-Object { $_.Name -eq "UEFI Firmware/BIOS"})
                if($xServerFW.Length -gt 0)
                {
                    $serverDataPoint.SetCurrentVersion($xServerFW[0].Version)
                }
                else
                {
                ...
                if (-not [String]::IsNullOrEmpty($serverDataPoint.CurrentVersion))
                {
                    ...
                    if ($serverDataPoint.CurrentVersion -match "`-(\d+\.\d+)")
                    {
                        ...
                        $serverDataPoint.SetCurrentVersion($Matches[1])
                    }
            ...
            while($a -lt $vmHosts.Length)
            {
                ...
                if(-not [String]::IsNullOrEmpty($serialNumber))
                {
                    ...
                    if($null -eq $server)
                    {
                        ...
                        $server.SetModel($vmH.Model)
                        $server.SetCurrentVersion(("{0} {1}.{2} [{3}]" -f @($vmH.ExtensionData.Hardware.BiosInfo.BiosVersion, $vmH.ExtensionData.Hardware.BiosInfo.MajorRelease, $vmH.ExtensionData.Hardware.BiosInfo.MinorRelease, $vmH.ExtensionData.Hardware.BiosInfo.ReleaseDate)).Replace(" . ", " "))
                ...
                if($null -ne $server)
                {
                    $server.SetOperatingSystem("VMware ESXi {0}" -f @($vmH.Version))
                }
                else
                {
...
function Collect-CDOTData
{
    ...
    while($a -lt $invCfg.Filers.CDOT.Controllers.Length)
    {
        ...
        try
        {
            ...
            $cdotController = Connect-NcController -Name $invCfg.Filers.CDOT.Controllers[$a] -Credential $cdotCredential -Transient:$true -ErrorAction Stop
        }
        catch
        {
        }
        ...
        # Handle the exception for ADC's cluster serial number.  The cluster license key was entered incorrectly and resulted in a duplicate serial
        #   number with APL-CDOTCLST01.
        if($clusterDP.Name -ne "ADC-CDOTCLST01")
        {
            $clusterDP.SetSerialNumber($cluster.ClusterSerialNumber)
        }
        else
        {
            $clusterDP.SetSerialNumber("1-80-131486")
        }
...
function Collect-SevenModeData
{
    ...
    while($a -lt $invCfg.Filers.SM.Nodes.Length)
    {
        ...
        {
            [Log]::Warning("Failed to connect to 7-mode filer {0}." -f @($invCfg.Filers.SM.Nodes[$a]))
            $a++
            continue   # Skip to the next controller.
        }
        ...
        $d.SetSerialNumber($nodeInfo.SystemSerialNumber)
        if($smController.Version -match "\w+ \w+ ([^\s]+)")
        {
            $d.SetCurrentVersion($Matches[1])
        }
        $d.SetOperatingSystem("DataONTAP 7-Mode")
        $d.SetManufacturer($nodeInfo.VendorId)

        # Append '-7M' to 7-Mode filer model so they different than Clustered Mode filers of the same model.  This is to facilitate
        #    tracking version numbers, and Clustered vs 7-Mode version is different, even though, for example, FAS2520s can run either.

        $d.SetModel("{0}-7M" -f @($nodeInfo.SystemModel))
                    ...
                    if(-not [String]::IsNullOrEmpty($device.sysDescr))
                    {
                        ...
                        if($descrParts.Length -gt 0)
                        {
                            ...
                            if($version.Length -eq 1)
                            {
                                ...
                                {
                                    $d.SetOperatingSystem("JUNOS")
                                    $d.SetCurrentVersion($Matches[1])
                                }
                            }

                            $d.SetModel(@($descrParts[0] -split " ")[0].ToUpper())
                        }
                    }
                ...
                "Riverbed Technology"
                {
                    ...
                    if($devInv.Length -gt 0)
                    {
                        ...
                        if($devInv.Length -eq 1)
                        {
                            ...
                            {
                                $d.SetModel($Matches[1])
                            }

                            $version = @($devInv[0].softwareRev -split " ")
                            if($version -gt 1)
                            {
                                $d.SetCurrentVersion($version[1])
                            }
                        }
                    }
...
while($a -lt $requiredModules.Length)
{
    ...
    {
        [Log]::Trace("Importing module {0}" -f @($requiredModules[$a]))
...
<#
  The following lines must be executed before DBConnectionMYSQL.ps1 is sourced in.  I don't have any documentation to back up my theory,
...
# Write the export using UTF-8-BOM
[System.IO.File]::WriteAllLines($exportFileName, $exportData, $utf8bom)
