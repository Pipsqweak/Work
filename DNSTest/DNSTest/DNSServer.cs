    ...
    public class DNSServer
    {
        ...
        public class DNSDomain
        {
            ...
            public bool ReverseZone
            {
                get
                {
                    if (((_wmiObject["Reverse"] is Boolean) && ((bool) _wmiObject["Reverse"])) || _wmiObject["Reverse"].ToString() == "1")
                    {
                        return true;
                    }
                    else
                    ...
                    catch (Exception ex)
                    {
                        if (temporaryException != null)
                        {
                            throw new ApplicationException("There were two exceptions, the primary failure" +
                                " was an exception that is encapsulated in this message however additionally " +
        ...
        public DNSServer(string server)
        {
            ...
            String mp = String.Format(@"\\{0}\Root\MicrosoftDNS", server);
            _scope = new ManagementScope(mp, co);
        ...
        public DNSRecord[] GetHostARecord(string hostName)
        {
            string
                query = String.Format("SELECT * FROM MicrosoftDNS_AType WHERE OwnerName = '{0}'", hostName);
        ...
        public DNSRecord[] GetHostPTRRecord(string hostName)
        {
            string
                query = "SELECT * FROM MicrosoftDNS_PTRType WHERE (ContainerName = '10.in-addr.arpa')";
                // query = String.Format("SELECT * FROM MicrosoftDNS_PTRType WHERE (NOT ContainerName LIKE '%RootHints%') AND (NOT ContainerName LIKE '%Cache')");
