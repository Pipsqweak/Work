...
do
{
    ...
    try
    {
        ...
        else
        {
            ...
            elseif($verify.IsPresent)
            {
                ...
                $myProposal = $proposalList.Where({ ($_.Status -notin @("Complete","Errored")) -and ($_.Discovered -eq "TRUE") -and ($_.Exported -eq "TRUE") -and ($_.Uploaded -eq "TRUE") -and ($_.FlatSetProcessed -eq "TRUE") -and ($_.Verified -eq "FALSE")}) | Select-Object -First 1
                if($null -ne $myProposal)
                {
                    $myProposal.Status = "Verifying"
                    $myProposal.Verified =  "Processing"
    ...
    {
        # $myProposal
        @("FolderGUID","pwProjectPath","Name","Description","Status","Discovered","Exported","Uploaded","FlatSetProcessed","Verified","Folders","Documents","Size","FlatSets","Notes").ForEach({
