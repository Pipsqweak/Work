...
<#
    When calling LogTrace, if the traceLevel is -le $Script:TraceLevel, then the message is logged.  If $Script:DoDebugging -eq $true, then the message is also output to the console.
#>
$Script:TraceLevel = -1
...
function LogTrace
{
    ...
    if($traceLevel -le $Script:TraceLevel)
    {
        LogOutput -IndentLevel $IndentLevel -Color Cyan -Prefix ("TRACE{0,2:D2}" -f @($traceLevel)) -Message $Message -NoNewLine:$NoNewLine -NewLine:$NewLine
    }
}
...
function LogDebug
{
    ...
    if($Script:DoDebugging)
    {
        LogOutput -IndentLevel $IndentLevel -Color DarkYellow -Prefix "DBG" -Message $Message -NoNewLine:$NoNewLine -NewLine:$NewLine
    }
}
...
function LogError
{
    ...
    {
        LogOutput -IndentLevel $IndentLevel -Color Red -Prefix "EXCEPTION" -Message $exceptionMsg -NoNewLine:$NoNewLine -NewLine:$NewLine
    }
