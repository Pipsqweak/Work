...
<#
    ...
    Builds a recursive dictionary based on the supplied CSV file used to translate "indirect" paths to "more" direct paths.  All POWER Engineers shares are hosted on NetApp filers.
        further, each SVM which hosts CIFS shares will have a Shares$ administrative share which encompasses all other shares.  For example:


        CDC-CDOTCLST01:CDC-SVMA01
            Volume: JP_vol_Shares
            Share Name: Shares$
            JunctionPath: /Shares

            Volume: JP_vol_PW
            Share: PW
            JunctionPath /Shares/PW

            Volume: vol_SMB_PW_dms_active_01
            Share: PW_Active_01$
            JunctionPath: /Shares/PW/Prod/DMS_Active

    This layout allows me to scan all folders and files under \\CDC-SVMAFS01\PW_Active_01$ indirectly via \\CDC-SVMAFS01\Shares$\PW\Prod\DMS_Active.

    Therefore, when a folder or file is checked indirectly, I use the dictionary to find a more direct path.

    So the following path will be translated as indicated.

        From: \\?\UNC\CDC-SVMAFS01\Shares$\PW\Prod\DMS_Active\lab_storage\dms00052\AllDGNLinkSets.qry
          To: \\?\UNC\cdcfs1\PW_Active_01$\lab_storage\dms00052\AllDGNLinkSets.qry

        \\?\UNC\CDC-SVMAFS01\Shares$\PW\Prod\DMS_Active has been replaced with the more direct: \\?\UNC\cdcfs1\PW_Active_01$

    Each node in the dictionary contains 4 properties:

        CIFSServer: The name of the CIFS server the node refers to.
        FS1Alias: An "FS1" alias if the SVM has one.
        Translation: A more direct path route to this path (if one exists)
        Children: A dictionary representing all the sub-shares hosted under this node.

    Using the examples above:

    root:
        Key: CDC-SVMAFS01
            CIFSServer: CDC-SVMAFS01
            FS1Alias: cdcfs1
            Translation: (null)
            Children:
                Key: Shares$
                    ...
                    Children:
                        Key: PW
                            CIFSServer: CDC-SVMAFS01
                            FS1Alias: cdcfs1
                            Translation: CDC-SVMAFS01\PW
                            Children:
                                Key: Prod
                                    CIFSServer: CDC-SVMAFS01
                                    FS1Alias: cdcfs1
                                    Translation: (null)   -- There is no direct share for \\?\UNC\CDC-SVMAFS01\Shares$\PW\Prod -- but we do need to track it since there are shares below it.
                                    Children:
                                        Key: DMS_Active
                                            CIFSServer: CDC-SVMAFS01
                                            FS1Alias: cdcfs1
                                            Translation: CDC-SVMAFS01\PW_Active_01$
                                            Children: {empty}

    Using the keys: CDC-SVMAFS01 -> Shares$ -> PW -> Prod -> DMS_Active
        $Global:pathTranslationDictionary["CDC-SVMAFS01"].Children["Shares$"].Children["PW"].Children["Prod"].Children["DMS_Active"]
    we arrive at a translation of: CDC-SVMAFS01\PW_Active_01$

    Therefore, we can replace "CDC-SVMAFS01\Shares$\PW\Prod\DMS_Active" in the orginal path with "CDC-SVMAFS01\PW_Active_01$" to get a more direct share path.

        From: \\?\UNC\CDC-SVMAFS01\Shares$\PW\Prod\DMS_Active\lab_storage\dms00052\AllDGNLinkSets.qry
          To: \\?\UNC\CDC-SVMAFS01\PW_Active_01$\lab_storage\dms00052\AllDGNLinkSets.qry

    And if there is an FS1Alias, we can further translate to:

        \\?\UNC\cdcfs1\PW_Active_01$\lab_storage\dms00052\AllDGNLinkSets.qry

    The function TranslatePath uses this dictionary to perform the actual translation.  See the description and comments there.
#>
function BuildTranslationDictionaryFromCSV
{
    ...
    try
    {
        ...
        <#
        @($allCifsShares | Where-Object { (($_.CifsServer -eq $forServer) -or ($_.FS1Alias -eq $forServer)) -and ($_.ShareName -ne "shares`$") }).ForEach({
            $tFrom = "\{0}{1}\" -f @($_.CifsServer, $_.Path -replace '/shares/','/Shares$/') -replace '/','\'
            $tTo = "\{0}\{1}\" -f @($_.CifsServer, $_.ShareName)
            $Global:translationMap.Add($tFrom,$tTo)
        })
        #>
...
function BuildTranslationDictionaryFromCSVTest1
{
    [CmdLetBinding()]
    Param(
        [Parameter(Mandatory=$true,Position=0)]
    ...
    try
    {
        ...
        # Get key for fs1alias: ($Global:pathTranslationDictionary.GetEnumerator() | Where-Object { $_.Value.FS1Alias -eq "cdcfs1" }).Key

        # Dictionary used to translate paths based on \\servername\shares$\... to \\servername\directShareName\...
        $Global:pathTranslationDictionary = [System.Collections.Generic.SortedDictionary[[String],[Object]]]::new([System.StringComparer]::OrdinalIgnoreCase)

        # Temporary dictionary key = 'long' path value = direct path (\\servername\sharename not included)
        #   Technically, this dictionary could be used, but I believe all the comparisons required would make it too slow...
        #   Using a SortedList allows direct access to the .Keys[$idx]...
        $translationMap = [System.Collections.Generic.SortedList[[String],[String]]]::new([System.StringComparer]::OrdinalIgnoreCase)

        <#
            $Global:maxTranslationDepth: The maximum sub-folders in any 'long' path after Shares$ which results in a more direct share.  This represents the maximum
                number of dictionary entries/sub-folder names which need to be checked to either reach the correct translation, or fail.

                ex:
                    long path: \\CDC-SVMAFS01\Shares$\PW\Prod\DMS_Active_04\active_dms_storage05\d3467705\4A.1_Centerline_Identification.pdf
                    direct path: \\CDC-SVMAFS01\PW_Active_04$\active_dms_storage05\d3467705\4A.1_Centerline_Identification.pdf

                    PW, Prod and DMS_Active_04 are the sub-folders names after Shares$ which are eliminated by the PW_Active_04$ share, so $Global:maxTranslationDepth -eq 3
        #>
        #$Global:maxTranslationDepth = -1
        $byAlias = $false

        if(-not [String]::IsNullOrEmpty($forServer))
        {
            # Get the import records for the requested server
            $shareRecs = @($allCifsShares | Where-Object { (($_.CifsServer -eq $forServer) -or ($_.FS1Alias -eq $forServer)) -and ($_.ShareName -ne "shares`$") })
            $byAlias = ($shareRecs.Count -gt 0) -and ($shareRecs[0].FS1Alias -eq $forServer)
        } `
        else
        {
            $shareRecs = @($allCifsShares | Where-Object { $_.ShareName -ne "shares`$" })
        }

        $uniqueCifsServers = @($shareRecs | Select-Object -Unique -ExpandProperty CifsServer | Sort-Object)

        $i = 0
        while($i -lt $uniqueCifsServers.Length)
        {
            $cifsServerRecs = $shareRecs.Where({$_.CifsServer -eq $uniqueCifsServers[$i] })
            if($cifsServerRecs.Count -gt 0)
            {
                $translationMap.Clear()
                $cifsServerRecs.ForEach({
                    $tFrom = $_.Path -replace '/shares/','' -replace '/','\'
                    $tTo = $_.ShareName
                    $translationMap.Add($tFrom,$tTo)
                })

                # Create top level entry for this CIFS server...
                $d = "" | Select-Object FS1Alias, Children, MaxTranslationDepth
                $d.MaxTranslationDepth = -1
                $d.FS1Alias = $cifsServerRecs[0].FS1Alias
                $d.Children = [System.Collections.Generic.SortedDictionary[[String],[Object]]]::new([System.StringComparer]::OrdinalIgnoreCase)
                $Global:pathTranslationDictionary.Add($uniqueCifsServers[$i], $d)

                $serverDict = $Global:pathTranslationDictionary[$uniqueCifsServers[$i]]
                $b = 0
                while($b -lt $translationMap.Keys.Count)
                {
                    $longPath = $translationMap.Keys[$b]
                    $forServer = $cifsServerRecs[0].CifsServer
                    if($byAlias)
                    {
                        $forServer = $cifsServerRecs[0].FS1Alias
                    }
                    $translateFrom = "\{0}\Shares$\{1}" -f @($forServer, $longPath)
                    $pathParts = $longPath.Split(@('\'), [System.StringSplitOptions]::RemoveEmptyEntries)

                    # Track the maximum sub-folders after 'Shares$'
                    if($pathParts.Length -gt $serverDict.MaxTranslationDepth)
                    {
                        Write-Host ("{0}" -f @(($pathParts -join "\")))
                        $serverDict.MaxTranslationDepth = $pathParts.Length
                    }

                    # Build the dictionary from the deepest sub-folder ...
                    $a = $pathParts.Length - 1

                    # Always start adding at [$uniqueCifsServers[$i]].Children...
                    $t = $serverDict.Children
                    while($a -ge 0)
                    {
                        if(-not $t.ContainsKey($pathParts[$a]))
                        {
                            $d = "" | Select-Object Children, TranslateFrom, TranslateTo
                            $d.Children = [System.Collections.Generic.SortedDictionary[[String],[Object]]]::new([System.StringComparer]::OrdinalIgnoreCase)
                            if(($a -eq 0) -and (-not [String]::IsNullOrEmpty($translationMap[$longPath])))
                            {
                                $d.TranslateFrom = $translateFrom
                                $d.TranslateTo = "\{0}\{1}" -f @($forServer, $translationMap[$longPath])
                            }
                        ...
                        {
                            Write-Host ("Dictionary already contains: {0} for {1}." -f @($pathParts[$a], $_))
                        }
                        $t = $t[$pathParts[$a]].Children
...
function SetTranslation
{
    ...
    if(($idx -lt $pathParts.Length) -and ($subFolderDict.ContainsKey($pathParts[$idx])) -and (-not [String]::IsNullOrEmpty($subFolderDict[$pathParts[$idx]].Translation)))
    {
        $currentTranslation.DirectShare = "\\{0}" -f @($subFolderDict[$pathParts[$idx]].Translation)
        $currentTranslation.TranslatedPath = $pathToTranslate -replace [Regex]::Escape(($pathParts[0..$idx] -join "\")), $subFolderDict[$pathParts[$idx]].Translation
    }
}

function TranslatePathv1($pathToTranslate)
{
    ...
    {
        # I don't think this is necessary ... Can't add the same key more than once, so why check to see if there is more than one entry with the same key???
...
<#
    TranslatePath uses $Global:pathTranslationDictionary built by BuildTranslationDictionaryFromCSV to attempt to translate a path into a more direct share path.
#>
function TranslatePath
{
    ...
    $d = $null
    if(-not [String]::IsNullOrEmpty($pathToTranslate))
    {
        $tDict = $Global:pathTranslationDictionary
        $tPath = RemoveLongUNCPath $pathToTranslate

        # Split the path we are translating into it's sub-directories and file name.  These are the "keys" used to translate the path.
        $pathParts = $tPath.Split(@('\'), [System.StringSplitOptions]::RemoveEmptyEntries)

        # Make sure there is at least a server and share name available.  (server name = [0], share name = [1])
        if($pathParts.Length -ge 1)
        {
            $d = "" | Select-Object TranslatedPath, DirectShare

            # Initialize .DirectShare to \\server\share...
            $d.DirectShare = "\\{0}" -f @(($pathParts[0..1] -join "\"))

            # Initialize .TranslatedPath to the path we are trying to translate from
            $d.TranslatedPath = $pathToTranslate

            <#
                See the description above BuildTranslationDictionaryFromCSV for more complete definition of the dictionary...

                The following loop continues deeper into the dictionary as long as there are entries for the sub-directories and the sub-directory
                    dictionary has children of its own.

                Each time through the loop, if there is a translation available at a node, $d is updated accordingly.

                When the loop terminates (or do not even loop once)
            #>
            $a = 0
            while(($a -lt $pathParts.Length) -and $tDict.ContainsKey($pathParts[$a]) -and ($tDict[$pathParts[$a]].Children.Keys.Count -gt 0))
            {
                # The following line just ensures $pathParts[$a] is EXACTLY the same as it's matching key ... same case..
                #   I believe this is so the Regex::Escape function will work properly.
                $pathParts[$a] = @($tDict.Keys | Where-Object { $_ -eq $pathParts[$a] })[0]
                ...
                #SetTranslation -subFolderDict $tDict -pathParts $pathParts -idx $a -pathToTranslate $pathToTranslate -currentTranslation $d

                # Continue deeper into the dictionary looking for a more direct share...
                $tDict = $tDict[$pathParts[$a]].Children
                $a++
            }

            # In case the above loop never executes and a translation is available, try to set it...
            if(($a -lt $pathParts.Length) -and ($tDict.ContainsKey($pathParts[$a])) -and (-not [String]::IsNullOrEmpty($tDict[$pathParts[$a]].Translation)))
            {
                $d.DirectShare = "\\{0}" -f @($tDict[$pathParts[$a]].Translation)
                $d.TranslatedPath = $pathToTranslate -replace [Regex]::Escape(($pathParts[0..$a] -join "\")), $tDict[$pathParts[$a]].Translation
            ...
            # Are we translating a long UNC path?
            if((-not [String]::IsNullOrEmpty($d.DirectShare)) -and ($pathToTranslate -match "^\\\\\?\\unc\\"))
            {
                # If so, then make .DirectShare a long UNC path
                $d.DirectShare = AddLongUNCPath $d.DirectShare
            }

            # If there is an entry with a matching CIFS server and contains an FS1 alias, then change the (CIFS) server names to the FS1 aliases.
            if($Global:pathTranslationDictionary.ContainsKey($pathParts[0]) -and (-not [String]::IsNullOrEmpty($Global:pathTranslationDictionary[$pathParts[0]].CIFSServer)) -and (-not [String]::IsNullOrEmpty($Global:pathTranslationDictionary[$pathParts[0]].FS1Alias)))
            {
                $d.DirectShare = [regex]::Replace($d.DirectShare, ("\\{0}\\" -f @($Global:pathTranslationDictionary[$pathParts[0]].CIFSServer)), ("\{0}\" -f @($Global:pathTranslationDictionary[$pathParts[0]].FS1Alias)))
                $d.TranslatedPath = [regex]::Replace($d.TranslatedPath, ("\\{0}\\" -f @($Global:pathTranslationDictionary[$pathParts[0]].CIFSServer)), ("\{0}\" -f @($Global:pathTranslationDictionary[$pathParts[0]].FS1Alias)))
            }
        ...
        {
            # Nothing... can't translate a path that doesn't contain at least a server name and a share name...
            $d = $null
        }
    } `
    else
    {
        # Can't translate an empty path...
    }

    return $d
}

function TranslatePathTest1
{
    [CmdletBinding()]
    param (
        [Parameter()]
    ...
    if(-not [String]::IsNullOrEmpty($pathToTranslate))
    {
        $tDict = $Global:pathTranslationDictionary["__ROOT__"].Children
        if($null -ne $tDict)
        {
            $tPath = RemoveLongUNCPath $pathToTranslate

            # Split the path we are translating into it's sub-directories and file name.
            $pathParts = $tPath.Split(@('\'), [System.StringSplitOptions]::RemoveEmptyEntries)
            ...
            if($pathParts.Length -ge 2)
            {
                ...
                $startAt = $Global:maxTranslationDepth + 2 # Remember, $Global:maxTranslationDepth represents the maximum LENGTH of a path to translate from, without the server and share name, so adjust here since $pathToTranslate still contains them.

                # Don't start testing beyond the end of $pathParts
                if($startAt -gt $pathParts.Length)
                {
                    $startAt = $pathParts.Length
                }

                $a = $startAt - 1   # Remember, $startAt represent a length, so the first index to check is 1 less...

                # First find the first subfolder with an entry in the translation dictionary
                $translationFound = $false
                while(($a -gt 1) -and (-not $tDict.ContainsKey($pathParts[$a])))
                {
                    $a--
                }

                # At this point, either $a -le 1 OR $tDict.ContainsKey($pathParts[$a])
                if($a -ge 2)
                {
                    # Capture the matching dictionary entry
                    $tDict = $tDict[$pathParts[$a]]

                    # if $a -eq 2 then we are as far back in the path as we can go ...
                    if($a -gt 2)
                    {
                        # We found a matching entry, but are not down to the last subfolder to check...
                        # From here on, for this to be a matching translation, all other subfolders have to match...


                        #   HERE
                    }
                }
                if($a -gt 2)   # If so, we found a dictionary entry matching a subfolder
                {
                    $translationFound = $true   # maybe
                    # From here on, for this to be a matching translation, all other subfolders have to match...
                    $b = $a - 1 # We already checked $pathParts[$a]

                    $tDict2 = $tDict[$pathParts[$a]].Children

                    while(($b -gt 1) -and ($null -ne $tDict2) -and ($tDict2.ContainsKey($pathPaths[$b])))
                    {
                        $b--
                        $tDict2 = $tDict2[$pathParts[$b]].Children
                    }

                    do
                    {
                        $b--
                        if($null -ne $tDict2.Children)
                        {
                            $tDict2 = $tDict2.Children
                        }
                    }
                    while(($b -gt 1) -and ($null -ne $tDict2) -and ($tDict2.ContainsKey($pathParts[$b])))
                } `
                else
                {
                    # No matching entry...
            ...
            {
                Write-Host -ForegroundColor Red "Not path parts available."
            }
        } `
        else
        {
            Write-Host -ForegroundColor Red "No path translation dictionary available."
        }
...
function DumpExplicitACLs()
{
    ...
    {
        $Global:explicitACLRules | Export-Csv -NoTypeInformation -Delimiter "`t" -Path $saveToPath
    }
    else
    {
...
function DumpExceptions()
{
    ...
    {
        $Global:directoryExceptions | Set-Content -Path $saveToPath
    }
    else
    {
        Write-Host "No exceptions found."
    }
}
...
function DumpUntranslated()
{
    ...
    if($Global:untranslatedPaths.Length -gt 0)
    {
        $Global:untranslatedPaths.ToString() | Set-Content -Path $saveToPath
    }
    else
    {
        Write-Host "No untranslated paths found."
    }
}
...
function ListDirectory
{
    ...
    if($null -ne $di)
    {
        if($di -is [System.IO.DirectoryInfo])
        {
            ...
            if($null -eq $pathTranslation)
            {
                # Likely don't need this, but it would be good to flesh it out...
            }

            # Not able to translate the full path to a direct share...
            $untranslated = $pathTranslation.DirectShare -match "\\shares\$"
            ...
            if($untranslated)
            {
                # Track all the untranslated paths
                [void] $Global:untranslatedPaths.AppendLine($di.FullName)
            }
            ...
            if($null -ne $diDirectories)
            {
                ...
                if($null -ne $diFiles)
                {
                    ...
                    $diFiles | ForEach-Object {
                        ...
                        <# Let's not track down to the files... if the file is untranslated, then its folder will also be untranslated...
                        if($untranslated)
                        {
                            # Track all the untranslated paths
                            $Global:untranslatedPaths.AppendLine($fi.FullName)
                        }
                        #>
...
function SaveData($folderToSaveTo,$cifsServer)
{
    try
    {
        ...
        $exceptionsDataFile = "{0}\exceptions.txt" -f @($serverFolder)
        DumpExceptions -saveToPath $exceptionsDataFile

        $untranslatedPathsDataFile = "{0}\untranslated.txt" -f @($serverFolder)
        DumpUntranslated -saveToPath $untranslatedPathsDataFile
    }
    catch
    {
...
function LD
{
    ...
    $Global:translationMap = [System.Collections.Generic.SortedList[[String],[String]]]::new([System.StringComparer]::OrdinalIgnoreCase)
    ...
    $Global:untranslatedPaths = [System.Text.StringBuilder]::new()
...
function MergeUntranslatedPathsData
{
    [CmdletBinding()]
    param (
        [Parameter(Mandatory=$true, Position=0)]
        ...
        [ValidateNotNull()]
        [System.Object[]]
        $untranslatedPathsDataFiles
    )

    $savePath = "{0}\CombinedUntranslatedPathsData.csv" -f @(($dataFolder.TrimEnd("\")))
    $a = 0
    while($a -lt $untranslatedPathsDataFiles.Length)
    {
        try
        {
            $fod = Get-Content -Path $untranslatedPathsDataFiles[$a].FullName
            try
            {
                Add-Content -Path $savePath -Value $fod
            }
            catch
            {
                Write-Host -ForegroundColor Red ("Failed to add untranslated path data from {0} to: {1}." -f @($untranslatedPathsDataFiles[$a].FullName, $savePath))
            }
        }
        catch
        {
            Write-Host -ForegroundColor Red ("Failed to read untranslated paths data from: {0}." -f @($untranslatedPathsDataFiles[$a].FullName))
        }
...
function CombineFileAgeData
{
    ...
    try
    {
        ...
        $untranslatedPathsDataFiles = @($files | Where-Object { $_.Name -eq "untranslated.txt" })
        MergeUntranslatedPathsData -dataFolder $dataFolder -fileExplicitACLDataFiles $untranslatedPathsDataFiles
    }
    catch
    {
