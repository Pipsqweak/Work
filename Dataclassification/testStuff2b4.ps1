...
function TranslatePathv2
{
    ...
    # Before all else, we need a translation dictionary...
    if($null -ne $Global:pathTranslationDictionary)
    {
        # Change from \\?\UNC\Server\Share to \\Server\Share if needed
        $tmpPathToTranslate = RemoveLongUNCPath $pathToTranslate

        $pathParts = $tmpPathToTranslate.Split('\', [System.StringSplitOptions]::RemoveEmptyEntries)

        # ALWAYS need at least a server name and a share name...
        if($pathParts.Length -ge 2)
        {
            if($pathParts[1] -eq "shares`$")
            {
                # $pathParts[x..2] will be what needs to be translated ... [1] = Shares$, [0] = Server Name
                $translationDictionary = $null
                $serverName = $pathParts[0]

                # Does the translation dictionary contain an entry for the server name?
                if($Global:pathTranslationDictionary.ContainsKey($serverName))
                {
                    # Use the dictionary entry based on the server name...
                    $translationDictionary = $Global:pathTranslationDictionary[$serverName]
                } `
                else
                {
                    # See if the dictionary has an entry which has an FS1Alias that matches the server name...
                    $l = $Global:pathTranslationDictionary.GetEnumerator() | Where-Object { $_.Value.FS1Alias -eq $serverName }

                    # I could use the following, but I'd lose access to the .Key ...  LATER: if I don't need $serverName anymore, I might change this...
                    #     $l = $Global:pathTranslationDictionary.Values | Where-Object { $_.FS1Alias -eq $serverName }
                    if($null -ne $l)
                    {
                        $serverName = $l.Key
                        $translationDictionary = $l.Value
                    } `
                    else
                    {
                        # Likely an error...
                ...
                # Ok, so do we have a dictionary to translate against?
                if($null -ne $translationDictionary)
                {
                    # Since I built the translation dictionary from the deepest subfolder up to (but not including Shares$) I can stop looking as soon as a subfolder does not match...
                    $a = $translationDictionary.MaxTranslationDepth + 1
                } `
                else
                {
                    # This is an error...server name nor possible FS1 alias found...
                }
            } `
            else
            {
                # Error: Malformed path... Shares$ exists in $pathToTranslate, but there doesn't appear to be a server name...
            }
            ...
            {
                # $pathParts does not contain Shares$ ... so likely don't need to translate it...
            }
        } `
        else
        {
